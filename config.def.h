...
static const unsigned int systrayspacing  = 6;      /* systray spacing between icons */
...
static const char *fonts[]                = {       /* fonts is self-explanatory */
  ...
  "JetBrains Mono:style=Regular:size=14:antialias=true:autohint=true",
  /*"Fantasque Sans Mono:size=15:antialias=true:autohint=true",*/
};
...
static const char *tags[] = { "󰆍", "󰊯", "󰓓", "󰊖", "󰙯", "󰢻", };  /* Tags declaration <- this also
                                                                 defines the total amount of tags
                                                                 there are so be careful. */
static const Rule rules[] = { /*  <-- This are the Tag's Rules
    Run command xprop(1): {
      WM_CLASS  =   Instance, Class
      WM_NAME   =   Title
    }
    Use (Class, Instance, Title) to help dwm identify a specific client = (X program's window).
    Use (Mask) to spawn programs on a specific Tag (Left_num = (On/Off))<<(Right_num = Tag).
    Use (Float) to spawn programs on Floating window mode by default (1=On/0=Off).
    Use (Monitor) to spawn programs on a specific(fixed) Monitor, -1 = Selected.
    ...
    { "Steam",              "Steam",              "Connection Error", 1<<2,   1,      -1      },
    { "Steam",              "Steam",              "Steam",            1<<2,   1,      -1      },
    { "Steam",              "Steam",              "Friends List",     1<<2,   1,      -1      },
    { "Steam",              "Steam",              "Steam - News",     1<<2,   1,      -1      },
    { "Lutris",             "lutris",             "Lutris",           1<<3,   0,      -1      },
    { "discord",            "discord",            NULL,               1<<4,   0,      -1      },
    { "qpwgraph",           "qpwgraph",           NULL,               1<<5,   0,      -1      },
    { "easyeffects",        "easyeffects",        "EasyEffects",      1<<5,   0,      -1      },
...
static const Arg tagexec[]  = {
  SHCMD("st -e bash"),
  SHCMD("com.github.Eloston.UngoogledChromium"),

 ...
 * handling X events. In contrast to other X clients, a window manager selects for
 * SubstructureRedirectMask on the root window, to receive events about window (dis-)appearance.
 * Only one X connection at a time is allowed to select for this event mask.
 *
 * The event handlers of dwm are organized in an array which is accessed whenever a new event
 * has been fetched. This allows event dispatching in O(1) time.
 *
 * Each child of the root window is called a client, except windows which have set the
 * override_redirect flag. Clients are organized in a linked client list on each monitor,
 * the focus history is remembered through a stack list on each monitor.
 * Each client contains a bit array to indicate the tags of a client.
 *
 * Keys and tagging rules are organized as arrays defined in config.h.
...
enum { SchemeNorm, SchemeSel, SchemeTag, SchemeTag1, SchemeTag2, SchemeTag3, SchemeTag4,
  SchemeTag5, SchemeTag6, SchemeLayout, SchemeTitle, SchemeTitleFloat };  /* color schemes */
enum { NetSupported, NetWMName, NetWMState, NetWMCheck, NetSystemTray, NetSystemTrayOP,
  NetSystemTrayOrientation, NetSystemTrayOrientationHorz, NetWMFullscreen, NetActiveWindow,
...
int applysizehints (Client *c, int *x, int *y, int *w, int *h, int interact) {
	...
	*h = MAX(1, *h);
	...
	if (resizehints || c->isfloating || !c->mon->lt[c->mon->sellt]->arrange) {
		...
		if (c->mina > 0 && c->maxa > 0) { /* adjust for aspect limits */
...
void attachbottom (Client *c) {
	...
	c->next = NULL;
	if (below) below->next = c;
	else c->mon->clients = c;
}
...
void buttonpress (XEvent *e) {
      ...
      (focusonwheel || (ev->button != Button4 && ev->button != Button5)))
  {
	...
	if (ev->window == selmon->barwin) {
      ...
      x = (selmon->ww - statusw - getsystraywidth()) - 9;
			click = ClkStatusText;
      char *text, *s, ch;
      dwmblockssig = 0;
...
int drawstatusbar (Monitor *m, int bh, char* stext) {
	...
	p = text;
	i = -1, j = 0;
	while (stext[++i]) if ((unsigned char)stext[i] >= ' ') text[j++] = stext[i];
	text[j] = '\0';
	...
	while (text[++i]) {
		if (text[i] == '^' && !isCode) {
			...
			while (text[++i] != '^') {
				...
				} else if (text[i] == 'f') x += atoi(text + ++i);
			}
			text = text + i + 1;
			i=-1;
			isCode = 0;
...
void drawbar (Monitor *m) {
   ...
   * status is only drawn on selected monitor */
	if (m == selmon) tw = statusw = m->ww - drawstatusbar(m, bh, stext);
	resizebarwin(m);
	for (c = m->clients; c; c = c->next) {
		occ |= c->tags;
    ...
    drw_setscheme(drw, scheme[occ & 1 << i ?
        (m->colorfultag ? tagschemes[i] : SchemeSel) : SchemeTag]);
		drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i);
    /* if there are conflicts, just move these lines directly
     * underneath both 'drw_setscheme' and 'drw_text' :) */
    if (ulineall || m->tagset[m->seltags] & 1 << i) drw_rect(drw, x + ulinepad,
        bh - ulinestroke - ulinevoffset, w - (ulinepad * 2), ulinestroke, 1, 0);
		x += w;
	}
  ...
  drw_setscheme(drw, scheme[SchemeLayout]);
	...
	if ((w = m->ww - tw - stw - x) > bh) {
      ...
      if (selmon->colorfultitle) {
        for (i = 0; i < LENGTH(tags); i++)
          if (selmon->sel->tags & 1 << i) drw_setscheme(drw, scheme[tagschemes[i]]);
      } else {
...
void focus (Client *c) {
    ...
    if (!solitary(c)) {
      if (selmon->pertag->curtag)
        XSetWindowBorder(dpy, c->win, scheme[tagschemes[selmon->pertag->curtag - 1]][0].pixel);
    }
		setfocus(c);
	} else {
...
Atom getatomprop (Client *c, Atom prop) {
	...
	if (XGetWindowProperty(dpy, c->win, prop, 0L, sizeof atom, False, req, &da, &di, &dl, &dl, &p)
      == Success && p)
  {
		atom = *(Atom *)p;
...
long getstate (Window w) {
	...
	if (XGetWindowProperty(dpy, w, wmatom[WMState], 0L, 2L, False, wmatom[WMState], &real,
        &format, &n, &extra, (unsigned char **)&p) != Success) return -1;
	if (n != 0) result = *p;
	XFree(p);
	return result;
}
...
void grabbuttons (Client *c, int focused) {
	updatenumlockmask();
	{
		...
		if (!focused) XGrabButton(dpy, AnyButton, AnyModifier, c->win, False, BUTTONMASK,
	...
	updatenumlockmask();
  {
    unsigned int i, j;
      ...
      if ((code = XKeysymToKeycode(dpy, keys[i].keysym)))
        for (j = 0; j < LENGTH(modifiers); j++) XGrabKey(dpy, code, keys[i].mod | modifiers[j],
...
void monocle (Monitor *m) {
	...
	for (c = nexttiled(m->clients); c; c = nexttiled(c->next)) {
      ...
      resize(c, m->wx, m->wy, m->ww - 2 * c->bw, m->wh - 2 * c->bw, 0);
		else resize(c, m->wx, m->wy, m->ww, m->wh, False);
  }
}
...
void movemouse (const Arg *arg) {
	...
	if (XGrabPointer(dpy, root, False, MOUSEMASK, GrabModeAsync, GrabModeAsync, None,
        cursor[CurMove]->cursor, CurrentTime) != GrabSuccess) return;
	...
	do {
      ...
      case MotionNotify:
        ...
        if (!c->isfloating && selmon->lt[selmon->sellt]->arrange && (abs(nx - c->x) > snap
              || abs(ny - c->y) > snap)) togglefloating(NULL);
	...
	if (XQueryTree(dpy, root, &d1, &d2, &wins, &num)) {
      ...
      if (!XGetWindowAttributes(dpy, wins[i], &wa) || wa.override_redirect
		...
		for (i = 0; i < num; i++) { /* now the transients */
            ...
            || getstate(wins[i]) == IconicState)) manage(wins[i], &wa);
		}
		if (wins) XFree(wins);
	}
}
...
void setfocus (Client *c) {
  ...
  if (c->issteam) setclientstate(c, NormalState);
...
void setgaps (const Arg *arg) {
	switch(arg->i) {
      ...
      break;
		case GAP_RESET:
			if (selmon->pertag->curtag > 0) selmon->pertag->gappx[selmon->pertag->curtag] =
        gappx[selmon->pertag->curtag - 1 % LENGTH(gappx)];
			else selmon->pertag->gappx[0] = gappx[0];
      break;
    default:
      if (selmon->pertag->gappx[selmon->pertag->curtag] + arg->i < 0)
        selmon->pertag->gappx[selmon->pertag->curtag] = 0;
			else selmon->pertag->gappx[selmon->pertag->curtag] += arg->i;
	}
	arrange(selmon);
}
...
void setmfact (const Arg *arg) {  /* arg > 1.0 will set mfact absolutely */
	float f;
...
void sigdwmblocks (const Arg *arg) {
  ...
  if (!dwmblockspid) if (getdwmblockspid() == -1) return;
...
void tag (const Arg *arg) {
	if (selmon->sel && arg->ui & TAGMASK) {
		...
		focus(NULL);
		arrange(selmon);
		if (viewtonewtag && ((arg->ui & TAGMASK) != TAGMASK)) view(arg);
	}
}
...
void tile (Monitor *m) {
        ...
        c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
    {
      if (i < m->nmaster) {
        ...
        resize(c, m->wx + mw + m->pertag->gappx[m->pertag->curtag], m->wy + ty,
            m->ww - mw - (2*c->bw) - 2*m->pertag->gappx[m->pertag->curtag], h - (2*c->bw), 0);
    ...
    for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
      if (i < m->nmaster) {
        ...
        if (n == 1) resize(c, m->wx, m->wy, mw - (2*c->bw), h - (2*c->bw), 0);
        else resize(c, m->wx, m->wy + my, mw - (2*c->bw), h - (2*c->bw), False);
        my += HEIGHT(c);
      } else {
        h = (m->wh - ty) / (n - i);
...
void unmapnotify (XEvent *e) {
	...
	if ((c = wintoclient(ev->window))) {
		...
		/* KLUDGE! sometimes icons occasionally unmap their windows, but do _not_ destroy them.
...
void updateclientlist () {
	...
	XDeleteProperty(dpy, root, netatom[NetClientList]);
	for (m = mons; m; m = m->next)
		for (c = m->clients; c; c = c->next) XChangeProperty(dpy, root, netatom[NetClientList],
...
int updategeom (void) {
	...
	if (XineramaIsActive(dpy)) {
      ...
      if (isuniquegeom(unique, j, &info[i]))
		...
		XFree(info);
		nn = j;
    for (i = n; i < nn; i++) {  /* new monitors if nn > n */
          ...
          || unique[i].width != m->mw || unique[i].height != m->mh)
      {
		...
		for (i = nn; i < n; i++) {  /* removed monitors if n > nn */
			for (m = mons; m && m->next; m = m->next);
			while ((c = m->clients)) {
				dirty = 1;
				m->clients = c->next;
...
void updatenumlockmask (void) {
	...
	for (i = 0; i < 8; i++)
      ...
      if (modmap->modifiermap[i * modmap->max_keypermod + j] ==
          XKeysymToKeycode(dpy, XK_Num_Lock)) numlockmask = (1 << i);
...
void updatesystrayicongeom (Client *i, int w, int h) {
  ...
  if (i) {
    ...
    else i->w = (int)((float)rh * ((float)w / (float)h));
    i->y = i->y + vertpadbar / 2;
    applysizehints(i, &(i->x), &(i->y), &(i->w), &(i->h), False);
    if (i->h > rh) {  /* force icons into the systray dimensions if they don't want to */
      if (i->w == i->h) i->w = rh;
      else i->w = (int)((float)rh * ((float)i->w / (float)i->h));
      i->h = rh;
		}
	}
}
...
void updatesystrayiconstate (Client *i, XPropertyEvent *ev) {
	...
	if (!showsystray || !i || ev->atom != xatom[XembedInfo] ||
      !(flags = getatomprop(i, xatom[XembedInfo]))) return;
...
void updatesystray (void) {
  ...
  ypad = vp;
	...
	if (!systray) { /* init systray */
      ...
      die("fatal: could not malloc() %u bytes\n", sizeof(Systray));
		systray->win = XCreateSimpleWindow(dpy, root, x - xpad, m->by + ypad, w, bh, 0, 0,
	...
	w = w ? w + systrayspacing : 1; x -= w;
	XMoveResizeWindow(dpy, systray->win, x - xpad, m->by + ypad, w, bh);
	wc.x = x - xpad; wc.y = m->by + ypad; wc.width = w; wc.height = bh;
...
Client * wintoclient (Window w) {
	Client *c;
	Monitor *m;
	for (m = mons; m; m = m->next) for (c = m->clients; c; c = c->next) if (c->win == w) return c;
	return NULL;
}
...
Monitor * wintomon (Window w) {
	...
	for (m = mons; m; m = m->next) if (w == m->barwin) return m;
...
void zoom (const Arg *arg) {
	...
	if (c == nexttiled(selmon->clients)) if (!c || !(c = nexttiled(c->next))) return;
	pop(c);
}

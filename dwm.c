...
Atom getatomprop (Client *c, Atom prop) {
	...
	Atom da, atom = None;
	/* FIXME getatomprop should return the number of items and a pointer to the stored data
...
static int isuniquegeom (XineramaScreenInfo *unique, size_t n, XineramaScreenInfo *info) {
	while (n--) if (unique[n].x_org == info->x_org && unique[n].y_org == info->y_org
...
void monocle (Monitor *m) {
	...
	for (c = nexttiled(m->clients); c; c = nexttiled(c->next))
      ...
      resize(c, m->wx, m->wy, m->ww - 2 * c->bw, m->wh - 2 * c->bw, 0);
		else resize(c, m->wx - c->bw, m->wy, m->ww, m->wh, False);
}
...
void movemouse (const Arg *arg) {
	...
	do {
		...
		case MotionNotify:
          ...
          && (abs(nx - c->x) > snap || abs(ny - c->y) > snap)) togglefloating(NULL);
...
void sigdwmblocks (const Arg *arg) {
  ...
  if (errno == ESRCH) if (!getdwmblockspid()) sigqueue(dwmblockspid, SIGUSR1, sv);
}
...
/* There's no way to check accesses to destroyed windows, thus those cases are ignored
 * (especially on UnmapNotify's). Other types of errors call Xlibs default error handler,
 * which may call exit. */
